...
const Register = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!formData.name.trim()) { toast.error('Nama lengkap wajib diisi!'); return; }
        if (!formData.email.trim()) { toast.error('Email wajib diisi!'); return; }
        if (!formData.phone.trim()) { toast.error('Nomor HP wajib diisi!'); return; }
        if (!formData.password) { toast.error('Kata sandi wajib diisi!'); return; }
        if (formData.password.length < 6) { toast.error('Kata sandi minimal 6 karakter!'); return; }
        if (formData.password !== formData.confirmPassword) { toast.error('Konfirmasi kata sandi tidak cocok!'); return; }
        setIsLoading(true);
                ...
                <div className="bg-white rounded-2xl p-10 shadow-[0_4px_16px_rgba(0,0,0,0.08)]">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-bold text-gray-900">Nama Lengkap<span className="text-accent"> *</span></label>
                            <input 
                                type="text" 
                                name="name" 
                                value={formData.name} 
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-bold text-gray-900">No. Hp<span className="text-accent"> *</span></label>
                            ...
                            <p className="text-xs text-gray-500 mt-1">Pilih negara dan masukkan nomor telepon</p>
                        </div>
                        <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-bold text-gray-900">Konfirmasi Kata Sandi<span className="text-accent"> *</span></label>
                            <div className="relative">
                                <input 
